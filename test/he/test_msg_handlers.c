...
#include "network.h"
#include "memory.h"
// We need the real conn.h to handle event callbacks, and mock_fake_dispatch for the transitive
// linkage
#include "conn.h"
...
he_return_code_t event_cb_pong(he_conn_t *conn, he_client_event_t event, void *context) {
  call_counter++;
...
void test_msg_handler_ping_fails_when_not_in_connected_state(void) {
  ...
  TEST_ASSERT_EQUAL(HE_ERR_INVALID_CLIENT_STATE, ret);
}

void test_msg_handler_ping_successful(void) {
  conn->state = HE_STATE_ONLINE;

  wolfSSL_write_ExpectAnyArgsAndReturn(HE_SUCCESS);
...
void test_msg_handler_pong(void) {
  // Check the function triggers the PONG callback
  conn->event_cb = event_cb_pong;
...
void test_msg_auth_auth_access_granted(void) {
  ...
  // We're not testing Wolf here
  wolfSSL_write_IgnoreAndReturn(SSL_SUCCESS);

  // We should get access denied and the call counter should be 2
  he_return_code_t res = he_handle_msg_auth(conn, empty_data, sizeof(empty_data));
