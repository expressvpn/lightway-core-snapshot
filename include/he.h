...
typedef enum he_return_code {
  ...
  /**
   * @brief This will be returned if a function was called against a client context that isn't in a
   * good state.
   *
   * For example this could happened when calling he_connect() on an already connected context.
   */
  HE_ERR_INVALID_CLIENT_STATE = -3,
...
/**
 * @brief Status codes for a Helium Lite client
 */
typedef enum he_client_state {
...
typedef enum he_client_event {
  ...
  HE_EVENT_PONG = 2,
  /// Client tried to send fragmented packets which were rejected as they are not supported by
  /// Helium
  HE_EVENT_CLIENT_SENT_FRAGMENTED_PACKETS = 3,
 ...
 * @see he_client_set_context Sets the value of the context pointer
 *
 * Whenever Helium changes state, this function will be called.
 */
typedef he_return_code_t (*he_state_change_cb_t)(he_conn_t *conn, he_client_state_t new_state,
                                                 void *context);

/**
 * @brief The prototype for the inside write callback function
 * @param client A pointer to the client context that triggered the callback
 ...
 * @param client AA pointer to the client context that triggered the callback
 * @param packet A pointer to the packet data
 * @param length The length of the entire packet in bytes
 * @param context A pointer to the user defined context
 * @see he_client_set_context Sets the value of the context pointer
...
typedef he_return_code_t (*he_event_cb_t)(he_conn_t *conn, he_client_event_t event, void *context);
 ...
 * he_client_get_nudge_time(), the host application can register this callback instead.
 *
 * @note Any pending timers should be reset with the value provided in the callback and there should
 * only ever be one timer per client context. Whilst excessive nudging won't cause Helium to
 ...
 * @param conn A pointer to the conn that triggered the callback
 ...
 * @param conn A pointer to the conn that triggered the callback
 * @param [out] A guaranteed valid pointer to a network_config_ipv4_t, to be populated the host
 * @param context A pointer to the user defined context
 * @see he_client_set_context Sets the value of the context pointer
 *
...
/**
 * @brief Data structure to hold all the state needed as a Helium Lite client
 *
 */
struct he_client {
  he_conn_t *conn;
...
static const size_t HE_PACKET_OVERHEAD = sizeof(he_msg_data_t) + sizeof(he_wire_hdr_t) +
